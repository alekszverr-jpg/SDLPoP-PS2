...
setting_type visuals_settings[] = {
		...
		{.id = SETTING_PS2_SCREEN_WIDTH, .style = SETTING_STYLE_NUMBER, .number_type = SETTING_BYTE,
				.linked = &ps2_screen_width, .min = 70, .max = 130,
				.text = "Screen width",
				.explanation = "Shrink or enlarge the picture width to fit the television."},
		{.id = SETTING_PS2_SCREEN_HEIGHT, .style = SETTING_STYLE_NUMBER, .number_type = SETTING_BYTE,
				.linked = &ps2_screen_height, .min = 70, .max = 130,
				.text = "Screen height",
				.explanation = "Shrink or enlarge the picture height to fit the television."},
...
void play_menu_sound(int sound_id) {
	#ifdef __PS2__
	// Menu feedback is intentionally silent on PS2. Repeated UI sounds can
	// interfere with the console's timing-sensitive audsrv stream, while menu
	// navigation remains fully visible on screen.
	(void)sound_id;
	return;
	#endif
	play_sound(sound_id);
